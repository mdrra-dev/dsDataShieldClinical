...
check_numericDS <- function(df) {
  check_range <- function(x, min = NULL, max = NULL, integer = FALSE, decimals = NULL) {
    # NA values are always allowed and never flagged as invalid
    invalid <- rep(FALSE, length(x))
    if (!is.null(min))
      invalid <- invalid | (!is.na(x) & x < min)
    if (!is.null(max))
    ...
    if (integer)
      invalid <- invalid | (!is.na(x) & x != round(x))
    if (!is.null(decimals))
      invalid <- invalid | (!is.na(x) & abs(x - round(x, decimals)) > .Machine$double.eps^0.5)
    any(invalid)
  }
